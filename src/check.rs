...
pub fn whnf_unfold(top_node: Rc<Expr>) -> (Rc<Expr>, Vec<Rc<Expr>>) {
...
#[inline]
pub fn fold_args(mut head: Rc<Expr>, mut args: Vec<Rc<Expr>>) -> Rc<Expr> {
...
#[inline]
pub fn whnf(node: Rc<Expr>) -> Rc<Expr> {
  let (head, args) = whnf_unfold(node);
  fold_args(head, args)
}

pub fn equal(expr_a: &Rc<Expr>, expr_b: &Rc<Expr>, unique: &mut usize) -> bool {
  ...
  match (&*head_a, &*head_b) {
    (Expr::Lam(_, _, a_dom, a_bod), Expr::Lam(_, _, b_dom, b_bod)) => {
      if equal(a_dom, b_dom, unique) {
        let f_var = Rc::new(Expr::FVar(Name::from_unique(*unique)));
    ...
    (Expr::FVar(uniq_a), Expr::FVar(uniq_b)) => *uniq_a == *uniq_b,
    (Expr::Sort(lvl_a), Expr::Sort(lvl_b)) => is_same_level(lvl_a, lvl_b),
...
pub fn is_proof_irrel(_expr_a: &Rc<Expr>, _expr_b: &Rc<Expr>) -> bool {
  // TODO
  false
}
