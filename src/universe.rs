...
pub fn combining(lvl: &Rc<Univ>, other: &Rc<Univ>) -> Rc<Univ> {
  match (&**lvl, &**other) {
    ...
    (Univ::Succ(lhs), Univ::Succ(rhs)) => {
      Rc::new(Univ::Succ(combining(lhs, rhs)))
    }
    _ => Rc::new(Univ::Max(lvl.clone(), other.clone())),
  }
}

pub fn is_same_level(lvl_a: &Rc<Univ>, lvl_b: &Rc<Univ>) -> bool {
...
pub fn leq_core(lvl_a: &Rc<Univ>, lvl_b: &Rc<Univ>, diff: i32) -> bool {
  match (&**lvl_a, &**lvl_b) {
    ...
    (Univ::IMax(a, b), _) if is_any_max(b) => match &**b {
      ...
      Univ::Max(x, y) => {
        let new_max = simplify(&Rc::new(Univ::Max(
          Rc::new(Univ::IMax(a.clone(), x.clone())),
          Rc::new(Univ::IMax(a.clone(), y.clone())),
        )));
        leq_core(&new_max, lvl_b, diff)
      }
    ...
    (_, Univ::IMax(x, y)) if is_any_max(y) => match &**y {
      ...
      Univ::Max(j, k) => {
        let new_max = simplify(&Rc::new(Univ::Max(
          Rc::new(Univ::IMax(x.clone(), j.clone())),
          Rc::new(Univ::IMax(x.clone(), k.clone())),
        )));
        leq_core(lvl_a, &new_max, diff)
      }
...
pub fn ensure_imax_leq(
  ...
  let closure =
    |subst: &Vec<(Rc<Univ>, Rc<Univ>)>, left: &Rc<Univ>, right: &Rc<Univ>| {
